...
// bundle must start, complete the initialize handshake, register every tool, and
// run self-contained (a non-inlined dep would crash the child with
// ERR_MODULE_NOT_FOUND; see tsup.config.ts noExternal and bundle.test.ts).
const distPath = join(process.cwd(), "..", "dist", "index.js");

// read.ts (36) + mylist.ts (7) + login.ts (2). Bump when a tool is added/removed.
const EXPECTED_TOOLS = 45;

test("e2e: built bundle runs standalone, handshakes, lists all tools, gates personal-list tools", async (t) => {
  ...
  const sandbox = join(tmpdir(), `mal-mcp-e2e-${process.pid}`);
