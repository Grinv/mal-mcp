...
export class RateLimiter {
  ...
  // `null` is an explicit "no acquisition has happened yet" sentinel — not a
  // reliance on real Date.now() always being far from 0 (true for any real
  // clock, but not for one mocked to start at/near the epoch in tests).
